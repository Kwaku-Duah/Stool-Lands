...
export async function seedPayableServices() {
  try {
    const existingPayableServices = await db.payableService.findMany();
    ...
    if (existingPayableServices.length === 0) {
      ...
      const createdPayableServices = await db.payableService.createMany({
